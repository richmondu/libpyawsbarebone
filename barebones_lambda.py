...
###############################################################################
CONFIG_AWS_SERVICE        = 'lambda'
CONFIG_AWS_REGION         = 'ap-southeast-1'

CONFIG_HTTP_METHOD        = 'POST'
CONFIG_HTTP_API           = '/2015-03-31/functions/FT900LambdaToSNS/invocations'
...
class lambda_barebones:
    ...
    def generateRequestLambda(self, message_to_send):
        return message_to_send

    def createRequestLambda(self, message_to_send):
        ...
        request_parameters = self.generateRequestLambda(message_to_send)
...
def main(args):
    ...
    message_to_send = {}
    message_to_send["topic"] = "arn:aws:sns:ap-southeast-1:773510983687:FT900SNStopic"
    message_to_send["subject"] = "subject"
    message_to_send["body"] = "body of message"
    message_to_send = json.dumps(message_to_send)
    print(message_to_send)

    client = lambda_barebones()
    request_input = client.createRequestLambda(message_to_send)
    client.connect()
    client.sendRequest(request_input)
    client.recvResponseLambda()
    client.close()
